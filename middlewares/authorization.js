...
const teamMemberAuth = (req, res, next) => {
    try {
        const token = req.headers['authorization'].split(' ')[1];
        jwt.verify(token, 'vacationcalendar', (error, decodedToken) => {
            if (decodedToken && decodedToken.role === 'TeamMember' || decodedToken.role === 'administrator') {
                req.userId = decodedToken.userID;
                next();
            } else {
                console.log(error);
...
const teamLeadAuth = (req, res, next) => {
    try {
        ...
        jwt.verify(token, 'vacationcalendar', (error, decodedToken) => {
            if (decodedToken && decodedToken.role === 'TeamLead' || decodedToken.role === 'administrator') {
                req.userId = decodedToken.userID;
                next();
            } else {
                console.log(error);
